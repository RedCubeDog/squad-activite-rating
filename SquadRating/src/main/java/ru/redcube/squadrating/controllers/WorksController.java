...
public class WorksController {
    ...
    @PostMapping("/works/saveWork/{workId}")
    public String updateWork(@PathVariable("workId") long id, HardWork hardWork, BindingResult result) {
        if (result.hasErrors()) {
            hardWork.setId(id);
            return "update-work";
        }
    ...
    @GetMapping("/works/edit/{workId}")
    public String showUpdateForm(@PathVariable("workId") long id, Model model) {
